...
# --- NUEVAS IMPORTACIONES PARA DATOS ESTRATÉGICOS ---
...
class ClienteSerializer(serializers.ModelSerializer):
    ...
    def get_gasto_total(self, obj):
        """
        Suma el total de todas las ventas no anuladas del cliente.
        Devuelve 0 si no hay ventas.
        """
        total = Ventas.objects.filter(cliente_venta=obj).exclude(estado_venta__nombre_estado='ANULADA').aggregate(
            total_gastado=models.Sum('total_venta')
        )['total_gastado']
        return total or 0

    def get_ultima_compra(self, obj):
        """
        Obtiene la fecha de la venta más reciente no anulada.
        Devuelve None si no hay ventas.
        """
        ultima = Ventas.objects.filter(cliente_venta=obj).exclude(estado_venta__nombre_estado='ANULADA').order_by('-fecha_venta').first()
        return ultima.fecha_venta if ultima else None

    def get_numero_de_compras(self, obj):
        """
        Cuenta el número de ventas no anuladas para el cliente.
        """
        return Ventas.objects.filter(cliente_venta=obj).exclude(estado_venta__nombre_estado='ANULADA').count()
