...
class ProveedorSerializer(serializers.ModelSerializer):
    ...
    # Le dice a DRF que use el EstadoSimpleSerializer para mostrar los detalles del estado al LEER.
    estado_proveedor = EstadoSimpleSerializer(read_only=True)
    ...
    numero_de_compras = serializers.SerializerMethodField()
    ...
    # --- MÉTODOS PARA CAMPOS ESTRATÉGICOS ---
    def get_gasto_total(self, obj):
        """
        Suma el total de todas las compras no canceladas al proveedor.
        """
        total = Compras.objects.filter(proveedor_compra=obj).exclude(estado_compra__nombre_estado='CANCELADA').aggregate(
            total_gastado=models.Sum('total_compra')
        )['total_gastado']
        return total or 0

    def get_ultima_compra(self, obj):
        """
        Obtiene la fecha de la compra más reciente no cancelada.
        """
        ultima = Compras.objects.filter(proveedor_compra=obj).exclude(estado_compra__nombre_estado='CANCELADA').order_by('-fecha_compra').first()
        return ultima.fecha_compra if ultima else None

    def get_numero_de_compras(self, obj):
        """
        Cuenta el número de compras no canceladas al proveedor.
        """
        return Compras.objects.filter(proveedor_compra=obj).exclude(estado_compra__nombre_estado='CANCELADA').count()
