...
const JefeControlStock = () => {
    ...
    // Filtro para el buscador DENTRO del modal de agregar stock
    ...
    const getStatus = (product) => {
        if (product.total_stock === 0) return { text: 'Sin Stock', className: 'bg-yellow-900 text-yellow-300', value: 'out' };
        if (product.is_low_stock) return { text: 'Stock Bajo', className: 'bg-red-900 text-red-300', value: 'low' };
    ...
    const tableFilteredProducts = useMemo(() => {
        return products.filter(p => {
            const searchMatch = (p.nombre_producto.toLowerCase().includes(tableSearchTerm.toLowerCase()) || 
                                 p.barcode.toLowerCase().includes(tableSearchTerm.toLowerCase()));
            const categoryMatch = tableSelectedCategory ? p.categoria.id_categoria === parseInt(tableSelectedCategory) : true;
            const statusMatch = tableSelectedStatus ? getStatus(p).value === tableSelectedStatus : true;
            return searchMatch && categoryMatch && statusMatch;
        });
    }, [products, tableSearchTerm, tableSelectedCategory, tableSelectedStatus]);

    const stats = useMemo(() => {
        const inventoryValue = tableFilteredProducts.reduce((acc, product) => acc + (product.precio_unitario_venta_producto * product.total_stock), 0);
        const uniqueProducts = tableFilteredProducts.length;
        const lowStockAlerts = tableFilteredProducts.filter(p => p.is_low_stock && p.total_stock > 0).length;
        const outOfStockProducts = tableFilteredProducts.filter(p => p.total_stock === 0).length;
        return { inventoryValue, uniqueProducts, lowStockAlerts, outOfStockProducts };
    }, [tableFilteredProducts]);
                            ...
                            <div className="p-4 md:p-5">
                                <form className="space-y-4" onSubmit={handleCreateProduct}>
                                    <div>
                                        ...
                                        <input type="text" name="nombre_producto" id="nombre_producto" value={newProduct.nombre_producto} onChange={handleNewProductChange} className="border text-sm rounded-lg block w-full p-2.5 bg-pr-dark-gray border-gray-500 placeholder-gray-400 text-white" placeholder="Ej: Gaseosa 500ml" required />
                                    </div>
                                    <div>
                                        <label htmlFor="barcode" className="block mb-2 text-sm font-medium text-white">SKU / Código de Barras</label>
                                        <input type="text" name="barcode" id="barcode" value={newProduct.barcode} onChange={handleNewProductChange} className="border text-sm rounded-lg block w-full p-2.5 bg-pr-dark-gray border-gray-500 placeholder-gray-400 text-white" placeholder="Ej: 7790010001234" required />
                                    </div>
                                    <div>
                                        <label htmlFor="categoria_producto" className="block mb-2 text-sm font-medium text-white">Categoría</label>
                                        <select name="categoria_producto" id="categoria_producto" value={newProduct.categoria_producto} onChange={handleNewProductChange} className="border text-sm rounded-lg block w-full p-2.5 bg-pr-dark-gray border-gray-500 text-white" required>
                                    ...
                                    <div>
                                        <label htmlFor="precio_unitario_venta_producto" className="block mb-2 text-sm font-medium text-white">Precio de Venta ($)</label>
                                        <input type="number" name="precio_unitario_venta_producto" id="precio_unitario_venta_producto" value={newProduct.precio_unitario_venta_producto} onChange={handleNewProductChange} className="border text-sm rounded-lg block w-full p-2.5 bg-pr-dark-gray border-gray-500 placeholder-gray-400 text-white" placeholder="0.00" required />
                                    </div>
                                    <div>
                                        <label htmlFor="low_stock_threshold" className="block mb-2 text-sm font-medium text-white">Umbral de Stock Bajo</label>
                                        <input type="number" name="low_stock_threshold" id="low_stock_threshold" value={newProduct.low_stock_threshold} onChange={handleNewProductChange} className="border text-sm rounded-lg block w-full p-2.5 bg-pr-dark-gray border-gray-500 placeholder-gray-400 text-white" required />
                                    </div>
