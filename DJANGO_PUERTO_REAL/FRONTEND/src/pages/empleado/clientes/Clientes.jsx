...
const Clientes = () => {
    ...
    if (loading) {
        return (
            <div>
                <EmpleadoHeader title="Gestión de Clientes" subtitle="Lista y administración de clientes" />
    ...
    return (
        <div>
            <EmpleadoHeader title="Gestión de Clientes" subtitle="Lista y administración de clientes" right={(
                <button 
                ...
                {filteredClients.length > 0 ? (
                    filteredClients.map((client) => (
                        <SectionCard
                            ...
                            icon={<div className="w-14 h-14 bg-pr-yellow rounded-full flex items-center justify-center text-pr-dark font-bold text-xl shrink-0">{getInitials(client)}</div>}
                            title={`${client.user_first_name} ${client.user_last_name}`}
                            description={client.user_email || 'Sin email'}
                            ariaLabel={`Cliente ${client.user_first_name} ${client.user_last_name}`}
                        >
