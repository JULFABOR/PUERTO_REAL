...
const StockHistoryModal = ({ isOpen, onClose, product }) => {
    ...
    useEffect(() => {
        if (isOpen && product) {
            const fetchHistory = async () => {
                ...
                setHistory([]);
                try {
                    // --- CAMBIO 1: Sintaxis de Axios GET ---
                    // Antes: const data = await apiClient(`/api/stock/historial-producto/...`);
                    // Ahora: apiClient.get(ruta_sin_api)
                    const response = await apiClient.get(`/stock/historial-producto/${product.id_producto}/`);
                    
                    // --- CAMBIO 2: Los datos están en response.data ---
                    const data = response.data;
                    setHistory(data.results || data || []);
                
                } catch (err) {
                    // --- CAMBIO 3: Manejo de errores de Axios ---
                    const errorMsg = err.response?.data?.detail || err.message || 'Error al cargar el historial.';
                    setError(errorMsg);
    ...
    const renderContent = () => {
        ...
        return (
            // --- (El JSX de la tabla estaba perfecto, sin cambios) ---
    ...
    return (
        // --- (El JSX del modal estaba perfecto, sin cambios) ---
        <div
