...
import apiClient from '@/api/apiClient'; // Ajusta ruta si es necesario
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faSpinner, faFloppyDisk, faTimes } from '@fortawesome/free-solid-svg-icons'; // Añadido faTimes
...
const FormInput = ({ name, label, value, onChange, error, type = 'text', required = false }) => (
    <div className="relative z-0 w-full">
        <input
            ...
            onChange={onChange}
            required={required}
            autoComplete="off" // Añadido para evitar autocompletado si no se desea
            className={`
        ...
        {/* Muestra el primer error si es un array */}
...
const EditProviderModal = ({ isOpen, onClose, provider, onSuccess, supplierStates }) => {
    // Estado inicial vacío, se llena en useEffect
    const [formData, setFormData] = useState({});
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [errors, setErrors] = useState({});

    // Carga datos del proveedor al abrir/cambiar
    useEffect(() => {
        if (provider && isOpen) {
            setFormData({
                ...
                // Guarda el ID del estado actual
                estado_proveedor_id: provider.estado_proveedor?.id_estado || '',
            });
            setErrors({}); // Limpia errores previos
        }
        // No resetea formData al cerrar para evitar parpadeos si se reabre rápido
    }, [provider, isOpen]);

    // Maneja cambios en los inputs y select
    const handleChange = (e) => {
        const { name, value } = e.target;
        setFormData(prev => ({ ...prev, [name]: value }));
        // Limpia el error específico del campo al modificarlo
        if (errors[name]) {
    ...
    const handleUpdateProvider = async (e) => {
        ...
        // Construye el payload con los campos correctos para la API
        const payload = {
            ...
            estado_proveedor_id: formData.estado_proveedor_id // Envía el ID del estado
        };

        // Validación simple en frontend para el estado
        ...
        try {
            // Usa PATCH para enviar solo los campos modificados (mejor si el backend lo soporta)
            await apiClient(`/api/compras/proveedores/${provider.id_proveedor}/`, {
                method: 'PATCH', // Cambiado a PATCH
                body: JSON.stringify(payload)
            });
            toast.success('¡Proveedor actualizado con éxito!');
            onSuccess(); // Llama a la función del padre para refrescar
            onClose(); // Cierra el modal
        } catch (error) {
            // Manejo de errores del backend
            if (error.data && typeof error.data === 'object') {
                setErrors(error.data);
                 // Muestra el primer error encontrado en un toast
                ...
                // Error genérico
                const errorMsg = error.data?.detail || 'No se pudo actualizar el proveedor.';
    ...
    // Renderizado del modal (con animaciones y estilos consistentes)
    return (
        <div /* Overlay */
            className={`
                ...
                <div className="relative rounded-lg shadow bg-pr-dark border border-pr-gray/20">
                    ...
                    <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-gray-600">
                        ...
                        <button type="button" onClick={onClose} className="text-gray-400 bg-transparent rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center hover:bg-gray-600 hover:text-white">
                            <FontAwesomeIcon icon={faTimes} /> {/* Icono de cierre */}
                        </button>
                    </div>
                    ...
                    <form className="p-4 md:p-5" onSubmit={handleUpdateProvider}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <FormInput
                                ...
                                required // Asumiendo que CUIT es obligatorio
                            />
                            <FormInput
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    name="estado_proveedor_id" // Nombre correcto para enviar el ID
                                    value={formData.estado_proveedor_id || ''} // Vinculado al ID en el estado
                                    onChange={handleChange}
                                    required
                                ...
                                >
                                    ...
                                    {/* Mapea las opciones desde la prop supplierStates */}
                        ...
                        {/* Botones de Footer */}
                        <div className="flex items-center justify-end space-x-4 pt-4 border-t border-gray-600"> {/* Añadido borde superior */}
                            <button
                                type="button"
                                onClick={onClose}
                            ...
                            <button
                                type="submit"
                                disabled={isSubmitting}
                                className="w-auto text-pr-dark bg-pr-yellow hover:bg-yellow-400 font-bold rounded-lg text-sm px-5 py-2.5 text-center disabled:bg-yellow-700 disabled:opacity-70 flex items-center justify-center min-w-[170px]" // Ancho mínimo para evitar cambio de tamaño
                            >
                                {isSubmitting ? (
                                    <>
